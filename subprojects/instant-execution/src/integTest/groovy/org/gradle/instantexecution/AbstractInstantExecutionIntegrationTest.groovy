...
class AbstractInstantExecutionIntegrationTest extends AbstractIntegrationSpec {
    ...
    // TODO distinguish those cases?
    //  - problems found without reaching max-problems
    //  - max-problems reached => doesn't make sense, should be the same as above
    //  - error during instant execution, cancelled storing (without collecting problems?)
    protected void expectInstantExecutionFailure(
        ...
        if (totalProblemsCount == 1) {
            assertThat(failure.error, containsString("Caused by: ${InstantExecutionProblemException.name}: ${uniqueProblems.first()}"))
        } else {
            (1..totalProblemsCount).each { problemNumber ->
                assertThat(failure.error, containsString("Cause $problemNumber: ${InstantExecutionProblemException.name}:"))
            }
        }
