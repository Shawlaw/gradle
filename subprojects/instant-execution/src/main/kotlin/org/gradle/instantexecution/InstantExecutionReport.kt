...
class InstantExecutionReport(
    ...
    fun add(problem: PropertyProblem) {
        ...
        if (problems.size >= startParameter.maxProblems) {
            // TODO unique problems, path to report
    ...
    fun runWithExceptionHandling(block: () -> Unit): Throwable? {
        try {
            ...
            when (e) {
                is InstantExecutionException -> return e
    ...
    fun instantExecutionExceptionForErrors(): Throwable? =
        errors()
            ...
            // TODO unique problems, path to report
            // TODO this is hiding all collected problems ... report them properly instead
            ?.let { errors -> InstantExecutionErrorsException(errors) }
    ...
    private
    fun warnings() =
        problems.asIterable().filterIsInstance<PropertyProblem.Warning>()
